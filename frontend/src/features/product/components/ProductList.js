...
import { selectAllProducts,fecthProductsByFiltersAsync,selectTotalItems,selectAllCategories,selectAllBrands,fecthCategoriesAsync,fecthBrandsAsync } from "../productSlice";
...
export default function ProductList() {
  ...
  const handleFilter = (e,section,option)=>{
    ...
    if(e.target.checked){
      ...
      newFilter[section.id] = newFilter[section.id].filter(e=>e!=option.value);
    }
    setFilter(newFilter);
  }
  ...
  useEffect(()=>{
    ...
    dispatch(fecthProductsByFiltersAsync({filter,sort,pagination}));
...
function MobileFilter({mobileFiltersOpen,handleFilter,setMobileFiltersOpen,filters}){
  return(
                        ...
                        {products.map((product) => (
                          <Link key={product.id} to={`/product_detail/${product.id}`}>
                            <div className="group relative border-solid border-2 p-2 border-gray-200">
                              ...
                              {/* TODO: product stock will not be needed when backend is implemented */}
                            </div>
                          </Link>
                        ))}
                      </div>
